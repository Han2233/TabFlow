...
| 版本 | 核心能力 | 可体验点 | 累计耗时 |
|------|---------|---------|---------|
| Step 1 | 脚手架 + 标签页列表 | 侧边栏看到所有标签页，点击跳转 | ~4 天 |
| Step 2 | 域名分组 + 手动分组 | 标签页自动按网站归组，可自建分组拖拽管理 | ~10 天 |
| Step 3 | 搜索 + 快捷键 + Popup | 快捷键搜索定位标签页 | ~13 天 |
| Step 4 | 工作区快照 | 一键保存/恢复整套标签页布局 | ~17 天 |
| Step 5 | 分窗口 + 时间视图 | 按分组自动拆分到多个窗口 | ~21 天 |
| Step 6 | 重复检测 + 休眠 | 自动发现重复标签页，释放内存 | ~26 天 |
| Step 7 | 关闭历史回收站 | 误关标签页可从历史中恢复 | ~31 天 |
| Step 8 | 后端 + AI 分类 | AI 一键智能整理标签页分组 | ~37 天 |
| Step 9 | 体验打磨 | 引导、撤销、主题、性能优化 | ~42 天 |
