...
| Step 3 | 搜索 + 快捷键 + Popup | 快捷键搜索定位标签页 | ✅ 完成 | ~13 天 |
| Step 4 | 工作区快照 | 一键保存/恢复整套标签页布局 | 🔜 进行中 | ~17 天 |
| Step 5 | 分窗口 + 时间视图 | 按分组自动拆分到多个窗口 | ⬜ 待开始 | ~21 天 |
| Step 6 | 重复检测 + 休眠 | 自动发现重复标签页，释放内存 | ⬜ 待开始 | ~26 天 |
| Step 7 | 关闭历史回收站 | 误关标签页可从历史中恢复 | ⬜ 待开始 | ~31 天 |
| Step 8 | 后端 + AI 分类 | AI 一键智能整理标签页分组 | ⬜ 待开始 | ~37 天 |
| Step 9 | 体验打磨 | 引导、撤销、主题、性能优化 | ⬜ 待开始 | ~42 天 |
